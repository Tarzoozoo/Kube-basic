...
kubectl version
# View Cluster Information (Server)
kubectl cluster-info
# Show all pods running in each cluster
...
# Show current cluster connection configuration (Cluster + Namespace)
...
```bash
# Create nginx app deployment (Create Pods)
...
# Expose service load balancer to nginx deployment port 80 (Forward port and get public IP)
kubectl expose deployment nginx --type LoadBalancer --port 80
# Wait to see public ip to be active and test it
kubectl get services -w

# Bind the public IP with the domain --> Web-app ready for scale
```
...
# Change nginx deployment to use apache image instead
kubectl set image deployment nginx nginx=httpd:2.4-alpine --record

# See change every 1 second
